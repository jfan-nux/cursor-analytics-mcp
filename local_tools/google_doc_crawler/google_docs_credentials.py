...
def get_google_docs_scopes():
    """Get the required Google Docs API scopes (read-only by default)."""
    return [
...
def get_google_docs_write_scopes():
    """Get the required Google Docs API scopes for read/write operations."""
    return [
...
def get_google_docs_write_credentials():
    """
    Get Google Docs credentials with write permissions.
    
    Similar to get_google_docs_credentials but uses write scopes and a separate token file.
    
    Returns:
        Authenticated credentials object with write permissions
    """
    if not GOOGLE_AUTH_AVAILABLE:
        raise ImportError("Google Auth libraries not available. Please install google-auth and google-auth-oauthlib")

    scopes = get_google_docs_write_scopes()
    ...
    if credentials_file:
        ...
        if os.path.exists(credentials_file):
            try:
                ...
                # Check if it's OAuth2 credentials (has "installed" key)
                if "installed" in creds_data:
                    return _get_oauth2_write_credentials_from_file(credentials_file, scopes)
                # Check if it's service account credentials (has "type": "service_account")
                elif creds_data.get("type") == "service_account":
                    return ServiceAccountCredentials.from_service_account_file(credentials_file, scopes=scopes)
                else:
...
def _get_oauth2_write_credentials_from_file(credentials_file: str, scopes: list):
    """Get OAuth2 write credentials from file path (uses separate token file)."""
    try:
        # Try to load existing token for write access
        config_dir = Path(__file__).parent.parent.parent / "config"
        token_path = config_dir / "google_docs_write_token.pickle"
        creds = None
        ...
        raise ValueError(f"Failed to authenticate with OAuth2 for write access: {e}")
