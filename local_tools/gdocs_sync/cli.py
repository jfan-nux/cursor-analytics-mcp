...
from auth import UnifiedAuthenticator
from config import load_config
from core.gdocs_client import GoogleDocsClient
from core.markdown_converter import MarkdownConverter
from core.image_handler import ImageHandler
from core.mapping_manager import MappingManager
from core.apps_script_client import AppsScriptClient

# Optional: Enhanced converter for full image support
try:
...
class MarkdownGDocsSync:
    ...
    def __init__(self, config_path: str = None):
        ...
        Args:
            config_path: Path to config file (optional, uses env vars by default)
        """
        self.config = load_config(config_path)
        self.mapping_manager = MappingManager()

        # Initialize authenticator
        self.authenticator = UnifiedAuthenticator(
    ...
    def _process_with_apps_script(self, doc_id: str, tab_id: str = None):
        ...
        """
        # Get Apps Script deployment ID from config
        deployment_id = (self.config.apps_script_id or '').strip()

        if not deployment_id:
            print("\n⚠️  Apps Script Deployment ID not configured.")
            print("   Tables will appear as [table]/[row] markers.")
            print("   Images will appear as [img][path] markers.")
            print("   Set GOOGLE_APPS_SCRIPT_ID in config/.env to enable processing.")
            return
        ...
        try:
            script_service = self.authenticator.get_script_service()
            script_client = AppsScriptClient(script_service, deployment_id)
            template_id = self.config.template_doc_id
    ...
    def export(self, markdown_path: str, open_browser: bool = True):
        ...
        # Get services
        docs_service = self.authenticator.get_docs_service()
        ...
        image_handler = ImageHandler(
            drive_service,
            markdown_dir,
            self.config.image_download_dir
        )
    ...
    def import_doc(self, markdown_path: str, backup: bool = True, with_images: bool = False,
                   output_file: str = None):
        """
        Import Google Doc back to markdown.

        Args:
            markdown_path: Path to linked markdown file (used to find doc mapping)
            backup: Whether to backup existing file
            with_images: Use enhanced converter to download images
            output_file: Output filename (default: use Google Doc title with --with-images)
        """
        ...
        # Use enhanced converter if requested and available
        if with_images:
            if not ENHANCED_CONVERTER_AVAILABLE:
                print("Warning: Enhanced converter not available, falling back to basic import")
                with_images = False
            else:
                print("Using enhanced converter with image download...")

        if with_images:
            # Enhanced import with images
            try:
                enhanced_converter = EnhancedGoogleDocConverter()
                enhanced_converter.initialize_services(self.authenticator.get_credentials())

                # Determine output directory (parent of markdown file)
                markdown_abs_path = os.path.abspath(markdown_path)
                output_dir = os.path.dirname(markdown_abs_path)
                ...
                result = enhanced_converter.convert_document_to_markdown(
                    doc_url=doc_url,
                ...
                if result.get('markdown_file'):
                    ...
                    if output_file:
                        # User specified custom output filename
                        if not output_file.endswith('.md'):
                            output_file += '.md'
                        target_file = Path(output_dir) / output_file
                    else:
                        # Default: use document title from enhanced converter
                        doc_title = result.get('title', 'imported_doc')
                        # Sanitize title for filename
                        sanitized_title = re.sub(r'[^\w\s-]', '', doc_title).strip()
                        sanitized_title = re.sub(r'\s+', ' ', sanitized_title)
                        target_file = Path(output_dir) / f"{sanitized_title}.md"

                    target_folder = target_file.parent
                    final_markdown_path = str(target_file)

                    if source_file != target_file and source_file.exists():
                        content = source_file.read_text(encoding='utf-8')

                        # Check if there's an images folder that needs to be handled
                        source_images_folder = source_folder / "images"
                        if source_images_folder.exists() and any(source_images_folder.iterdir()):
                            # Update image paths in markdown to include the document folder name
                            doc_folder_name = source_folder.name
                            # URL-encode spaces in folder name for markdown compatibility
                            from urllib.parse import quote
                            doc_folder_name_encoded = quote(doc_folder_name, safe='')
                            # Replace images/ with doc_folder_name/images/
                            content = re.sub(
                                r'!\[([^\]]*)\]\(images/',
                                f'![\\1]({doc_folder_name_encoded}/images/',
                                content
                            )
                        ...
                        # Clean up source folder only if empty (no images)
                        if source_folder.exists() and source_folder != target_folder:
                            try:
                                remaining_files = [p for p in source_folder.rglob('*') if p.is_file()]
                        ...
                        final_markdown_path = str(source_file)

                    # Clean up the original linked file if different from target
                    if Path(markdown_path).resolve() != target_file.resolve():
                        original_file = Path(markdown_path)
                        if original_file.exists() and original_file.stat().st_size == 0:
                            original_file.unlink()  # Remove empty placeholder file

                    images_downloaded = result.get('images_downloaded', 0)

                    # Update mapping to point to the new file
                    self.mapping_manager.add_mapping(
                        final_markdown_path,
                        doc_id,
                        doc_url,
                    ...
                    print(f"✓ Successfully imported to {final_markdown_path}")
                    if images_downloaded > 0:
                        print(f"  Images downloaded: {images_downloaded}")
                        # Show the actual images folder path
                        if source_folder.exists():
                            print(f"  Images folder: {source_folder}/images/")
                else:
                    print("✗ Enhanced conversion failed")

            except Exception as e:
                print(f"✗ Enhanced import failed: {e}")
                print("Try without --with-images flag for basic import")
                return
        else:
            # Basic import (original behavior)
            docs_service = self.authenticator.get_docs_service()
            drive_service = self.authenticator.get_drive_service()
...
Examples:
  ...
  md2gdocs import document.md --with-images  Import with full image download
  ...
  md2gdocs status document.md              Show sync status
    ...
    import_parser.add_argument('file', help='Linked markdown file (used to find doc mapping)')
    import_parser.add_argument('-o', '--output', help='Output filename (default: use Google Doc title)')
    import_parser.add_argument('--no-backup', action='store_true', help='Don\'t backup existing file')
    import_parser.add_argument('--with-images', action='store_true',
                               help='Use enhanced converter to download images (like crawl)')
    ...
    # Crawl command
    crawl_parser = subparsers.add_parser('crawl', help='Crawl Google Drive folder to markdown')
    crawl_parser.add_argument('folder_url', help='Google Drive folder URL')
    crawl_parser.add_argument('-o', '--output', required=True, help='Output directory')
    crawl_parser.add_argument('--subfolder', help='Optional subfolder path within the Drive folder')
    ...
    try:
        if args.command == 'export':
            ...
            sync.import_doc(args.file, backup=not args.no_backup, with_images=args.with_images,
            ...
            from crawlers.folder_crawler import GoogleDriveFolderCrawler
            import logging
            ...
            result = crawler.process_folder(
                ...
                subfolder_path=args.subfolder,
                local_output_path=args.output
            )

            print(f"\n✓ Crawl complete!")
            print(f"  Folders processed: {result.get('folders_processed', 0)}")
            print(f"  Documents converted: {result.get('documents_converted', 0)}")
